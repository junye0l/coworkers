...
- 한국어 기준 비속어 필터링에 대한 외부 라이브러리가 많이 없어 향후 확장성에 대하여 아쉬웠다.
  
- 또한 기본적으로 필터링해주는 단어 목록의 범위가 제한적이라 일일히 테스트 후 목록을 구축해줘야하는 제한사항들이 아쉬웠다.

- 다음에는 외부라이브러리가 아닌 직접 구현 방식으로 해보는 것도 좋을 것 같다는 생각이다.
